...
import { createLoanContract, getLoanContractFromConnector } from '../helpers/contractHelper';
...
const InfoContract = () => {
	...
	const [contract, setContract] = useState<Contract | null>(createLoanContract(hmy));
	const { account, connector, library } = useWeb3React();

	const getDonationStored = async () => {
		if (contract) {
			try {
				const donations = await contract.methods.allLoanRequests(0).call();
	...
	useEffect(() => {
		if (connector && library) {
			(async () => {
				const contract = await getLoanContractFromConnector(connector, library);
				setContract(contract);
			})();
		}
	}, [connector, library, setContract]);
