...
import { checkRedeem, getFractionalised, transferFT } from 'utils';
		...
		async (token: IFracNFT) => {
			try {
				...
				const res = await toast.promise(txPromise, {
					pending: token.readyToRedeem ? 'Redeeming NFT...' : 'Burning FTs to prepare NFT for redemption',
					success: 'Success!',
				...
				>
					...
					<Box display="flex" alignItems="center">
						...
						<Typography fontSize="0.75rem">
							Click on a fractionalised token to burn the related fungible tokens. Click on a ready token to redeem it.
						</Typography>
					</Box>
				</Typography>
			</Box>
					...
					{list?.map(token => {
						return (
							<NFTCard
								...
								uri={token.uri}
								primaryText={<Typography fontWeight="bold">Token #{token.id}</Typography>}
								secondaryText={
